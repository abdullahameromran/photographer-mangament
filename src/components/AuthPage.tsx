...
export function AuthPage() {
  ...
  const submit=async(e:React.FormEvent)=>{e.preventDefault();setError('');setNotice('');setLoading(true);try{if(password.length<6)throw new Error('كلمة المرور يجب أن تكون 6 أحرف على الأقل');if(mode==='login')await authApi.signIn(email,password);else{const result=await authApi.signUp(name,email,password);if(result.needsConfirmation)setNotice('تم إنشاء الحساب. افتح بريدك لتأكيد الحساب ثم سجّل الدخول.');}}catch(err){setError(err instanceof Error?err.message:'حدث خطأ غير متوقع');}finally{setLoading(false);}};
